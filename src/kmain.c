...
void kmain(){
	...
	while (1) {
			// O kmain fica girando aqui. Assim que a interrupção mudar a variável para 1, ele entra no IF.
			if (command_ready) {
				// Lemos o buffer preenchido pela interrupção
				size_t len = serial_read(cmd);
				
				// Garantimos que a string lida termine com o caractere nulo (para as funções de string funcionarem)
				if (len > 0 && cmd[len - 1] == '\n') {
				...
				if (strncmp(cmd, "uptime", 6) == 0) {
                    ...
                    // Conversão manual simples de string para inteiro (mini-atoi)
                    while (*p >= '0' && *p <= '9') {
                    ...
                    if (alarm_time_secs > 0) {
                        ...
                        serial_puts("Formato de alarme inválido.\n");
                    }
                } else {
					serial_puts("Comando não reconhecido\n");
				}
