...
void timer_irq_enable()
{
	...
	csr_set(CSR_SIE, (CSR_SIE_STIE)); // habilita a interrupção do timer (bit 5 do CSR sie)
}

void timer_irq_disable()
{
	// função para desabilitar a interrupção do timer, usando o CSR sie
	csr_clear(CSR_SIE, (CSR_SIE_STIE)); // desabilita a interrupção do timer (bit 5 do CSR sie)
}
